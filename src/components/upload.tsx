...
export default function Upload({ quarter }: { quarter: string }) {
  ...
  const { setQ1, setQ2, setQ3, setQ4 } = useContext(
    QuarterContext
  ) as QuarterContextType;

  const setForQuarter = useCallback(
    (val: string[]) => {
      if (quarter === "q1") {
        setQ1(val);
        return;
  ...
  const response = useInference(selectedFile, confidence);
      ...
      <div className="space-y-4 p-2">
        <div>
          <p className="text-black">
            When you upload an image using the file input below, the image will
            be sent to our image classifier model. The model will analyze the
            image and return predictions, which may include identifying objects,
            features, or other relevant classifications within the image. The
            results will be displayed along with the confidence levels of each
            prediction. Please ensure that the image is in a supported format
            (such as JPEG, PNG, etc.) before uploading.
          </p>
        </div>
        <div>
        ...
        <ol className="text-black list-decimal ml-8">
          ...
          <li>
            The image will be automatically uploaded and processed by the
            classifier.
          </li>
        </ol>
        <div>
          <p className="text-black">
            The <code>confidence</code> field allows users to specify a minimum
            confidence threshold, ensuring that only detections with a
            confidence score equal to or greater than the specified threshold
